...
class ExercisesAPIClientError(Exception):
...
class ValidationError(ExercisesAPIClientError):
...
class ExercisesAPIClient:
    ...
    VALIDATION_RULES = {"muscle": {"type": "string", "required": False}, "name": {"type": "string", "required": False}, "equipment": {"type": "string", "required": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the ExercisesAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises ExercisesAPIClientError: If API key is invalid
        """
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[ExercisesAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns ExercisesData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises ExercisesAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise ExercisesAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise ExercisesAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise ExercisesAPIClientError(f"Request failed: {str(e)}")
